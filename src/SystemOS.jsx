...
function Taskbar({ onLogout, isMenuOpen, toggleMenu, openWindows = [], activeWindowId, onWindowClick, onCloseWindow, getAppMetadata, theme, pinnedApps = [], onPinnedAppClick, onUnpinApp, onReorderPinned, taskbarOrder = [] }) {
    ...
    return (
        <>
            ...
            <div data-taskbar="true" className="absolute bottom-6 left-1/2 -translate-x-1/2 h-[60px] flex items-center justify-between px-3 z-[9990] transition-all duration-500 rounded-3xl" style={{ background: 'var(--os-glass)', border: '1px solid var(--os-border)', boxShadow: '0 20px 40px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', fontFamily: 'var(--os-font)' }}>
                <div className="flex items-center h-full gap-2">
                    <button onClick={toggleMenu} className="flex flex-col items-center justify-center w-[44px] h-[44px] rounded-xl transition-all group active:scale-95" style={{ background: isMenuOpen ? 'var(--os-hover)' : 'transparent', boxShadow: isMenuOpen ? 'inset 0 0 10px rgba(0,255,135,0.2)' : 'none' }}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5 transition-transform group-hover:scale-110" style={{ color: isMenuOpen ? 'var(--os-primary)' : 'var(--os-text)' }}><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" /></svg>
                    </button>
                            ...
                            return (
                                <div key={itemId} className="relative flex items-center justify-center" draggable
                                    ...
                                    <button onClick={() => isOpen ? onWindowClick(itemId) : onPinnedAppClick(itemId)}
                                        ...
                                        className="flex items-center justify-center w-[44px] h-[44px] rounded-xl transition-all group hover:-translate-y-1 active:scale-95"
                                        style={{ background: isActive ? 'var(--os-hover)' : 'transparent', cursor: 'pointer', boxShadow: isActive ? '0 4px 15px rgba(0,255,135,0.1)' : 'none' }}>
                                        <AppIcon type={meta?.iconType} className="w-6 h-6 drop-shadow-md transition-transform group-hover:scale-110" />
                                    </button>
                                    {isOpen && !isActive && (<div className="absolute -bottom-1 w-1.5 h-1.5 rounded-full pointer-events-none" style={{ background: 'var(--os-primary)', opacity: isMinimized ? 0.4 : 0.8, boxShadow: '0 0 6px var(--os-primary)' }} />)}
                                    {isActive && (<div className="absolute -bottom-1 w-3 h-1.5 rounded-full pointer-events-none" style={{ background: 'var(--os-primary)', opacity: 1, boxShadow: '0 0 8px var(--os-primary)' }} />)}
                                </div>
                            );
                        })}
                    </div>
                </div>
                <div className="flex items-center gap-2 h-full text-xs font-sans tracking-tight ml-4 pl-4" style={{ color: 'var(--os-text)', borderLeft: '1px solid var(--os-border)' }}>
                    <button onClick={(e) => { e.stopPropagation(); setIsActionCenterOpen(!isActionCenterOpen); }} className="flex items-center gap-3 h-[40px] px-3 rounded-xl transition-all active:scale-95" style={{ background: isActionCenterOpen ? 'var(--os-hover)' : 'transparent' }} onMouseEnter={(e) => !isActionCenterOpen && (e.currentTarget.style.background = 'var(--os-hover)')} onMouseLeave={(e) => !isActionCenterOpen && (e.currentTarget.style.background = 'transparent')}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-[16px] h-[16px]"><path d="M5 12.55a11 11 0 0 1 14.08 0" /><path d="M1.42 9a16 16 0 0 1 21.16 0" /><path d="M8.53 16.11a6 6 0 0 1 6.95 0" /><line x1="12" y1="20" x2="12.01" y2="20" /></svg>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-[16px] h-[16px]"><rect x="2" y="7" width="16" height="10" rx="2" ry="2" /><line x1="22" y1="11" x2="22" y2="13" /></svg>
                    </button>
                    <div className="flex flex-col items-end justify-center px-3 h-[40px] rounded-xl transition-all hover:bg-white/5 cursor-default leading-tight">
                        <span className="font-bold text-[13px]">{time.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</span>
...
function DraggableWindow({ id, title, isMinimized, isMaximized, isActive, zIndex, onFocus, onClose, onMinimize, onMaximize, children }) {
    ...
    return (
        <div ref={windowRef} className={windowClasses} style={{ zIndex, width: isMaximized ? undefined : sizeRef.current.w, height: isMaximized ? undefined : sizeRef.current.h, background: 'var(--os-window-bg)', border: `1px solid ${isActive ? 'var(--os-border-active)' : 'var(--os-border)'}`, borderRadius: isMaximized ? '0' : '1.25rem', boxShadow: isActive ? '0 30px 60px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.1)' : '0 10px 30px rgba(0,0,0,0.4)', backdropFilter: 'blur(10px)', fontFamily: 'var(--os-font)', willChange: 'transform, opacity', backfaceVisibility: 'hidden' }} onPointerDown={() => onFocus(id)}>
            {resizeHandles}
            <div className="h-12 flex items-center justify-between px-4 cursor-grab active:cursor-grabbing select-none shrink-0" style={{ background: 'transparent', borderBottom: '1px solid var(--os-border)' }} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
                <div className="flex items-center gap-3">
                    <div className="flex gap-2 mr-2 group/controls">
                        <button onClick={onClose} className="w-[14px] h-[14px] rounded-full flex items-center justify-center bg-[#ff5f56] border border-[#e0443e] overflow-hidden"><svg viewBox="0 0 24 24" stroke="black" strokeWidth="4" className="w-2.5 h-2.5 opacity-0 group-hover/controls:opacity-100 transition-opacity"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg></button>
                        <button onClick={onMinimize} className="w-[14px] h-[14px] rounded-full flex items-center justify-center bg-[#ffbd2e] border border-[#dea123] overflow-hidden"><svg viewBox="0 0 24 24" stroke="black" strokeWidth="4" className="w-2.5 h-2.5 opacity-0 group-hover/controls:opacity-100 transition-opacity"><line x1="5" y1="12" x2="19" y2="12" /></svg></button>
                        <button onClick={onMaximize} className="w-[14px] h-[14px] rounded-full flex items-center justify-center bg-[#27c93f] border border-[#1aab29] overflow-hidden"><svg viewBox="0 0 24 24" stroke="black" strokeWidth="4" className="w-2.5 h-2.5 opacity-0 group-hover/controls:opacity-100 transition-opacity"><path d="M13 5L19 5 19 11M5 19L11 19 11 13" /></svg></button>
                    </div>
                    <h3 className="font-bold tracking-[0.1em] text-[13px] opacity-80" style={{ color: 'var(--os-text)' }}>{title.replace('// VIEWING: ', '')}</h3>
                </div>
            </div>
...
function StartMenu({ isOpen, onClose, onLaunchApp, onLogout, hiddenIcons = [], onRestoreIcon, desktopFiles = [], renamedLabels = {} }) {
    ...
    return (
        <>
            ...
            <div ref={menuRef} className="absolute bottom-28 left-1/2 -translate-x-1/2 z-50 flex flex-col overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8),inset_0_1px_0_rgba(255,255,255,0.1)]" style={{ width: '560px', height: '520px', background: 'var(--os-menu-bg)', border: '1px solid var(--os-border)', borderRadius: '24px', backdropFilter: 'blur(40px)', fontFamily: 'var(--os-font)' }}>
...
const OS_THEMES = [
    { id: 'terminal', name: 'Terminal', desc: 'Hacker style', color: '#00ff87', bg: '#0c0c0b' },
...
const SettingsApp = ({ currentWallpaper, onApplyWallpaper, osTheme, onSetTheme }) => {
    ...
    return (
        <div className="flex h-full" style={{ color: 'var(--os-text)', flexDirection: isMobile ? 'column' : 'row' }}>
            <div className={isMobile ? 'flex flex-row shrink-0 overflow-x-auto gap-2 px-4 py-3' : 'w-56 shrink-0 flex flex-col gap-2 px-6 py-6'} style={{ background: 'var(--os-glass)', borderRight: isMobile ? 'none' : '1px solid var(--os-border)', backdropFilter: 'blur(10px)', zIndex: 10 }}>
                {!isMobile && <h3 className="mb-6 font-bold text-sm tracking-[0.2em] uppercase" style={{ color: 'var(--os-primary)', fontFamily: '"Space Grotesk", sans-serif' }}>SETTINGS</h3>}
                {sidebarItems.map(item => {
                    const isActive = activeTab === item.id;
                    return (
                        <button key={item.id} onClick={() => setActiveTab(item.id)} className="flex items-center gap-3 text-left text-xs transition-all duration-300" style={{ padding: '12px 14px', borderRadius: 24, border: isActive ? '1px solid var(--os-primary)' : '1px solid transparent', background: isActive ? 'color-mix(in srgb, var(--os-primary) 10%, transparent)' : 'transparent', color: isActive ? 'var(--os-text)' : 'var(--os-text-secondary)', fontFamily: '"Outfit", sans-serif', fontWeight: isActive ? 500 : 300, boxShadow: isActive ? '0 8px 24px color-mix(in srgb, var(--os-primary) 20%, transparent)' : 'none', whiteSpace: 'nowrap' }}>
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4" style={{ color: isActive ? 'var(--os-primary)' : 'inherit' }}><path strokeLinecap="round" strokeLinejoin="round" d={item.icon} /></svg>
                            {item.label}
                        </button>
                    );
                })}
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                {activeTab === 'themes' && (
                    <div className="flex-1 flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between px-8 pt-8 pb-4 shrink-0"><h3 className="font-bold text-lg tracking-[0.2em] uppercase" style={{ fontFamily: '"Space Grotesk", sans-serif', color: 'var(--os-text)' }}>OS Theme</h3></div>
                        <div className="flex-1 overflow-y-auto">
                            <div className="px-8 pb-8">
                                <div className={`grid gap-5 p-4 -m-4 ${isMobile ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 max-w-5xl mx-auto'}`}>
                                {OS_THEMES.map(t => {
                                    const isActive = osTheme === t.id; return (
                                        <div key={t.id} onClick={() => onSetTheme(t.id)} className="flex flex-col items-center gap-3 p-4 transition-all duration-300 cursor-pointer rounded-[20px] outline-none" style={{ background: isActive ? 'var(--os-glass)' : 'var(--os-hover)', boxShadow: isActive ? `0 0 0 1px ${t.color}, 0 10px 30px ${t.color}33, inset 0 0 20px ${t.color}11` : '0 0 0 1px var(--os-border)', transform: isActive ? 'translateY(-4px)' : 'translateY(0)' }}>
                                            <div className="w-full aspect-video rounded-lg overflow-hidden relative" style={{ background: t.bg, boxShadow: isActive ? `0 0 0 1px rgba(255,255,255,0.1), 0 0 15px ${t.color}40` : '0 0 0 1px rgba(255,255,255,0.1)' }}>
                                                <div className="absolute bottom-0 left-0 right-0 h-4" style={{ background: t.color, opacity: 0.2 }} />
                                                <div className="absolute top-2 left-2 w-5 h-3.5 rounded" style={{ background: t.color, opacity: 0.4 }} />
                                                <div className="absolute top-2.5 right-2.5 w-2 h-2 rounded-full" style={{ background: t.color }} />
                                                <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px)', backgroundSize: '10px 10px', pointerEvents: 'none' }} />
                                            </div>
                                            <span className="text-xs uppercase tracking-widest mt-2" style={{ color: isActive ? 'var(--os-text)' : 'var(--os-text-secondary)', fontFamily: '"Space Grotesk", sans-serif', fontWeight: isActive ? 600 : 400 }}>{t.name}</span>
                                            {!isMobile && <span className="text-[10px] tracking-wider" style={{ color: isActive ? t.color : 'var(--os-text-muted)', fontFamily: '"Outfit", sans-serif' }}>{t.desc}</span>}
                                        </div>
                                    );
                                })}
                            </div>
                ...
                {activeTab === 'background' && (
                    <div className="flex-1 flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between px-8 pt-8 pb-4 shrink-0">
                            <h3 className="font-bold text-lg tracking-[0.2em] uppercase" style={{ fontFamily: '"Space Grotesk", sans-serif', color: 'var(--os-text)' }}>Background</h3>
                            <button onClick={() => onApplyWallpaper(previewWallpaper)} className="px-6 py-2 transition-all rounded-full text-xs uppercase tracking-widest font-bold" style={{ border: '1px solid var(--os-primary)', background: 'color-mix(in srgb, var(--os-primary) 15%, transparent)', color: '#fff', boxShadow: '0 4px 15px color-mix(in srgb, var(--os-primary) 20%, transparent)', fontFamily: '"Space Grotesk", sans-serif' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--os-primary)'; e.currentTarget.style.boxShadow = '0 6px 20px color-mix(in srgb, var(--os-primary) 40%, transparent)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'color-mix(in srgb, var(--os-primary) 15%, transparent)'; e.currentTarget.style.boxShadow = '0 4px 15px color-mix(in srgb, var(--os-primary) 20%, transparent)'; }}>Apply Update</button>
                        </div>
                        <div className="flex-1 overflow-y-auto">
                            <div className="px-8 pb-8">
                                <div className="w-full max-w-3xl mx-auto mb-10 p-4 rounded-[24px]" style={{ background: 'var(--os-glass)', border: '1px solid var(--os-border)' }}>
                                <div className="flex justify-between items-center px-2 pb-3 mb-2" style={{ borderBottom: '1px solid var(--os-border)' }}>
                                    <span className="text-[11px] uppercase tracking-widest" style={{ color: 'var(--os-text-secondary)', fontFamily: '"Space Grotesk", sans-serif' }}>Monitor Preview</span>
                                    <span className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--os-primary)' }}>{previewWallpaper ? 'Active File' : 'System Default'}</span>
                                </div>
                                <div className="relative w-full rounded-[16px] overflow-hidden" style={{ aspectRatio: '16/9', border: '1px solid var(--os-border)', background: '#000', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' }}>
                                    {previewWallpaper ? <img src={previewWallpaper} alt="Preview" className="w-full h-full object-cover" /> : <div className="w-full h-full" style={{ background: `radial-gradient(circle at center, color-mix(in srgb, var(--os-primary) 20%, transparent) 0%, transparent 70%)` }} />}
                                    {/* Glass dock representation */}
                                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 h-8 w-1/3 rounded-full backdrop-blur-md" style={{ background: 'var(--os-hover)', border: '1px solid var(--os-border)' }}></div>
                                </div>
                            </div>
                            
                            <h4 className="text-[11px] uppercase tracking-[0.2em] mb-4 text-center" style={{ color: 'var(--os-text-secondary)', fontFamily: '"Space Grotesk", sans-serif' }}>Select Wallpaper</h4>
                            
                            <div className={`grid gap-4 p-4 -m-4 ${isMobile ? 'grid-cols-2' : 'grid-cols-3 lg:grid-cols-4 max-w-4xl mx-auto'}`}>
                                <label className="flex flex-col gap-3 cursor-pointer group transition-all">
                                    <div className="aspect-video w-full rounded-xl flex flex-col items-center justify-center relative transition-all duration-300" style={{ background: 'transparent', boxShadow: '0 0 0 2px var(--os-border) dashed' }} onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 0 0 2px var(--os-primary) dashed'; e.currentTarget.style.background = 'color-mix(in srgb, var(--os-primary) 5%, transparent)'; e.currentTarget.style.transform = 'translateY(-2px)'; }} onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 0 0 2px var(--os-border) dashed'; e.currentTarget.style.background = 'transparent'; e.currentTarget.style.transform = 'translateY(0)'; }}>
                                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'color-mix(in srgb, var(--os-primary) 10%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 4 }}>
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4" style={{ color: 'var(--os-primary)' }}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                                        </div>
                                        <input type="file" accept="image/*" className="hidden" onChange={handleFileUpload} />
                                    </div>
                                    <span className="text-[10px] uppercase tracking-[0.15em] text-center" style={{ color: 'var(--os-text-secondary)', fontFamily: '"Outfit", sans-serif' }}>Upload Local</span>
                                </label>
                                {customUploads.map((customUrl, index) => {
                                    const isSelected = previewWallpaper === customUrl; return (
                                        <div key={`custom-${index}`} onClick={() => setPreviewWallpaper(customUrl)} className="flex flex-col gap-3 cursor-pointer group transition-all">
                                            <div className="aspect-video w-full rounded-xl transition-all duration-300 overflow-hidden relative" style={{ boxShadow: isSelected ? '0 0 0 2px var(--os-primary), 0 10px 30px color-mix(in srgb, var(--os-primary) 30%, transparent)' : '0 0 0 1px var(--os-border)', transform: isSelected ? 'translateY(-4px)' : 'translateY(0)', opacity: isSelected ? 1 : 0.7 }}>
                                                <img src={customUrl} alt="Custom" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                                                <button onClick={(e) => handleDeleteCustomUpload(e, customUrl)} className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center bg-black/60 border border-red-500/50 text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500 hover:text-white"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                            </div>
                                            <span className="text-[10px] uppercase tracking-[0.15em] text-center" style={{ color: isSelected ? 'var(--os-primary)' : 'var(--os-text-secondary)', fontFamily: '"Outfit", sans-serif', fontWeight: isSelected ? 600 : 300 }}>User_{index + 1}</span>
                                        </div>
                                    );
                                })}
                                {WALLPAPERS.map(wp => {
                                    const isSelected = previewWallpaper === wp.url; return (
                                        <div key={wp.id} onClick={() => setPreviewWallpaper(wp.url)} className="flex flex-col gap-3 cursor-pointer group transition-all">
                                            <div className="aspect-video w-full rounded-xl transition-all duration-300 overflow-hidden relative" style={{ boxShadow: isSelected ? '0 0 0 2px var(--os-primary), 0 10px 30px color-mix(in srgb, var(--os-primary) 30%, transparent)' : '0 0 0 1px var(--os-border)', transform: isSelected ? 'translateY(-4px)' : 'translateY(0)', opacity: isSelected ? 1 : 0.7 }}>
                                                {wp.url ? <img src={wp.url} alt={wp.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" /> : <div className="w-full h-full" style={{ background: `radial-gradient(circle at center, color-mix(in srgb, var(--os-primary) 20%, transparent) 0%, transparent 70%)` }} />}
                                            </div>
                                            <span className="text-[10px] uppercase tracking-[0.15em] text-center" style={{ color: isSelected ? 'var(--os-primary)' : 'var(--os-text-secondary)', fontFamily: '"Outfit", sans-serif', fontWeight: isSelected ? 600 : 300 }}>{wp.name}</span>
                                        </div>
                                    );
                                })}
                            </div>
